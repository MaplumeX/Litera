...
describe("App — synced book files", () => {
  it("downloads a synced book on demand before opening it", async () => {
    const screen = render(<App />);
    await act(async () => {
      fireEvent.click(screen.getByText("open-book"));
    });
    ...
    const downloadIndex = commands.indexOf("sync_download_book_file");
    const contextIndex = commands.indexOf("get_book_open_context");
    expect(downloadIndex).toBeGreaterThanOrEqual(0);
    expect(contextIndex).toBeGreaterThan(downloadIndex);
    const downloadCall = invokeMock.mock.calls.find(
      ([cmd]) => cmd === "sync_download_book_file",
    );
    expect(downloadCall?.[1]).toEqual({ bookId: "book1" });
  });
});
