...
Footnote reference marks (`noterefCss` in `src/lib/reader-styles.ts`) are styled as a theme-aware academic superscript (`0.83em`, matching the browser `<sup>` default `font-size: smaller`, `vertical-align: super`, no underline, accent `#2563eb` light / `#6db4ff` dark, matching `THEME_CSS.dark` link color). Selectors: `a[epub|type~="noteref"]`, fallback `sup > a[href^="#"]`, plus `a[href] > sup` and `a[href]:has(> sup)` for real-world books that write references as `<a href><sup>1</sup></a>` (sup inside the link, often with a cross-file href like `part0032.xhtml#jz_4_130`). The child combinators keep sibling note-body markers (`<a id></a><sup>①</sup>`) unstyled; `:has(> sup)` de-underlines the wrapper link because a sup-side `text-decoration: none` cannot remove decoration propagated from the ancestor `a`. **Gotcha**: foliate parses chapter XHTML with `parseFromString(str, mediaType)` in XML mode, so `epub:type` is a namespaced attribute — the escaped literal form `a[epub\\:type]` matches **zero** elements in Chromium, and `[epub|type]` silently drops the whole rule unless the stylesheet starts with `@namespace epub url("http://www.idpf.org/2007/ops");`. `generateStylesCss` therefore emits that `@namespace` as the first statement. Do not add a default-namespace declaration (would force prefixing every `html`/`p`/`a` selector); only the prefixed `epub` namespace is declared. Do not use `a[href*="#fn"]` — it catches ordinary links to `#fn*` anchors.
