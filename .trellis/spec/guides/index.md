...
- [ ] You want to edit an already-imported book's title/author/cover/description/publisher/language/series — `update_book_metadata`; do not reuse `save_book_metadata` (import commit only). Cover `None` keeps the file; empty `Some` is invalid. Do not rewrite the EPUB. Sort the shelf in the WebView; do not add a `list_books` sort param (see backend `tauri-commands.md` "Scenario: update book metadata after import")
...
- [ ] You want Agent conversation ticks to fill the left rail — slots are `basis-2 grow-0 shrink` with rail `justify-center`; do not `flex-1`. When the rail is mounted, message scroll is `pl-12`, not overlay-on-`p-3` (see frontend `component-guidelines.md` "chat user-message outline is a workspace rail")
