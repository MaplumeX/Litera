...
- **Target selection is explicit**: `TAURI_TARGET_TRIPLE`, `TAURI_ENV_TARGET_TRIPLE`, `CARGO_BUILD_TARGET`, or `--target` selects the requested target. Host detection is only the local fallback, and a requested non-host target fails instead of relabeling a host executable. Release CI must stay on native runners — see [Desktop Release](./release.md).
